...
A similar result is given by~\cite{GW16} for the set $\QSV_{n}$ as a sub-poset of the Bruhat order (see Section~\ref{sec:QSV}), which simplifies our proof of Theorem~\ref{thm:excedancequotient}.  This leads to a kind of duality between these sub- and quotient orders of the Bruhat poset.
...
$
The number of noncrossing partitions of size $n$ is the $n$th Catalan number, $C_{n} = \frac{1}{n+1}\binom{2n}{n}$~\cite{S99}.
...
\end{enumerate}
The induced order is dual to the interval between the empty diagram and the staircase in Young's lattice. 
...
\node at (0, 3) (123) {\begin{tikzpicture} \draw (0,0)--(0,.2)--(.2,.2)--(.2,0)--(0,0); \end{tikzpicture}};
\node at (1.5, 1.5) (1b23) {\begin{tikzpicture} \draw (0,0)--(.2,0)--(.2,.4)--(0,.4)--(0,0);  \draw (.2,.2)--(0,.2); \end{tikzpicture}};
\node at (-1.5, 1.5) (12b3) {\begin{tikzpicture} \draw (0,0)--(0,.2)--(.4,.2)--(.4,0)--(0,0);  \draw (.2,.2)--(.2,0); \end{tikzpicture}};
\node at (0, 0) (1b2b3) {\begin{tikzpicture} \draw (0,0)--(0,.2)--(.4,.2)--(.4,0)--(0,0);  \draw (.2,0)--(.2,.4)--(0,.4)--(0,.2); \end{tikzpicture}};
\draw[thick] (1b2b3) -- (1b23);
\draw[thick] (1b2b3) -- (12b3);
\draw[thick] (1b23) -- (123);
\draw[thick] (12b3) -- (123);
...
\end{tikzpicture}
\end{center}
\caption{The Hasse diagrams of: $\QSV_{3}$ with the Bruhat order; $\NCP_{3}$ with $\preceq$; and the dual interval in the Young's lattice.
}
...
%\begin{enumerate}
 The results of~\cite{GW16} describe the Bruhat order of the set $\{\omega_0w\omega_0^{-1}\; |\; w \in \QSV_{n}\}$ rather than $\QSV_{n}$, where $\omega_0\in S_n$ denotes the Bruhat-maximal element $\omega_0\in S_n$. 
...
Two key intermediate results in the proof of Theorem~\ref{thm:excedancequotient} are that each excedance class $\CCC_{\lambda}$ contains unique Bruhat-minimal and Bruhat-maximal elements, and moreover these are respectively a $321$-avoiding permutation and the element $Q_{\lambda} \in \QSV_{n}$.  Combined with Theorem~\ref{thm:excedancequotient}, this implies the following corollary.  
...
\begin{rem}
Proposition~\ref{prop:321avoid} implicitly defines a bijection between $321$-avoiding permutations and noncrossing partitions.  This bijection is equivalent to one used by Zinno in~\cite{Z02} and Gobet; see~\cite{Gobet}.
\end{rem}
