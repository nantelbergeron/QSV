...
\begin{enumerate}
\item The results of~\cite{GobetWilliams} describe the Bruhat order of the set $\{\omega_0w\omega_0\; |\; w \in \QSV_{n}\}$ rather than $\QSV_{n}$, where $\omega_0w\omega_0$ is the conjugation by the top element $\omega_0\in S_n$.
As described in Remark~\ref{rem:QSVnoncrossing}, these are the non-crossing partitions associated with the cycle $(12\ldots n)$ instead of $(n\ldots 21)$. This conjugation is an automorphism of the Bruhat order, so the results are equivalent. It will play an interesting role throughout this paper.
...
\begin{rem}
There is an alternative notion for excedance classes. Instead, one can define $\EV'(w)$ as the set of $w_{i}$ such that $i< w_{i}$, and $\EP'(w)$ as the set of indices $i$ such that $i<w_{i}$. 
This second choice would also be viable and lead to equivalent results as presented here.

This is directly related to the automorphism described in Remark~\ref{rem:auto}, providing us with two dual solutions to the work presented here.
\end{rem}
...
\Nantel{what you think of remark below?} 
  \begin{rem}
   It is easy to observe that the order $\le_{L}$ on the set $S_{n}/\!\!\sim$ forms a lattice. 
  The least upper bound of $\CCC_{\lambda} \vee \CCC_{\mu}$ is $\CCC_{\nu}$ if and only if $Q_{\lambda}\vee Q_{\mu}$ is contained in $\CCC_{\nu}$.
   The lattice $(S_{n}/\!\!\sim,\le_{L})$ is closely related to the Tamari lattice, also known as the associahedron~\cite{Tamari}. 
   The two lattices have the same number of vertices and covers, and each face of the Tamari lattice, whether it is square or pentagonal, corresponds to an interval in $(S_{n}/\!\!\sim,\le_{L})$.

R. González D'León has proposed that this may be evidence that the lattice $(S_{n}/\!\!\sim,\le_{L})$ is part of a larger family of partial orders corresponding to triangulations of certain flow polytopes, as discussed in \cite{vBGMFY} and related references.
  \end{rem}
...
However, unlike the Bruhat order, the two posets are not equivalent, and the order on the quotient is more noteworthy in relation to our current and future research. 
The study of the weak order restricted to $\QSV_n$ is left for the interested reader. \end{rem}
...
\Nantel{I am here!  Things not to forget:
 ...
 I have also added remark in Section about the week order... have a look (I asked the AI to write it nicely haha!) }
