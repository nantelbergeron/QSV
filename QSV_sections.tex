...
We define am equivalence relation $\sim$ on the symmetric group $S_{n}$ using weak excedances and show that its equivalence classes are naturally indexed by noncrossing partitions.  
...
\begin{rem} The Tamari lattice (see~\cite{Tamari}) is a well known order on binary tree or any associated combinatorial objects. 
This lattice can thus be defined on noncrossing partitions and it has the following property. For any noncrossing partition $\lambda$ we define $\ell(\lambda)$ to be
the length of the shortest path from the identity to $\lambda$ is the Tamari lattice. In a sequel paper, we will study polynomial $m_\lambda$ of degree $\ell(\lambda)$
...
correspond to the (graded) quotient $R_n\big/\langle \QSym_{n}^{+} \rangle $.
\end{rem}

\begin{rem}
In light of Remark~\ref{rem:altECs} one may find that the image $\omega_0 \QSV_n\omega_0$ of the set $\QSV_n$ under the automorphism in Remark~\ref{rem:auto} has similar properties.
In particular, the polynomials $P_\alpha$ may be modified to polynomials $P'_\alpha$ that vanish on every permutation of $\omega_0 \QSV_n\omega_0$.
The top homogeneous component of $P'_\alpha$ would also be quasisymmetric, showing an analogue results for $\omega_0 \QSV_n\omega_0$.
This is closely related to the automorphisms of the ring of quasysymmetric functions (see for example~\cite{JWY}).
\end{rem}
