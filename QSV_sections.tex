...
\begin{enumerate}
\item The results of~\cite{GobetWilliams} describe the Bruhat order of the set $\{\omega_0w\omega_0^{-1}\; |\; w \in \QSV_{n}\}$ rather than $\QSV_{n}$, where $\omega_0\in S_n$ denotes the Bruhat-maximal element $\omega_0\in S_n$. \lucas{I removed \\ the description \\ of conjugation \\ and just \\ wrote it \\ as $\omega_0w\omega_0^{-1}$: \\ is this OK?}
...
\Lucas{I moved these down because together they broke the flow of the section a bit too much.}
...
\Lucas{I'm going to save edits on this for the last minute -- how we change it will depend a lot on whether we can prove $\le_{L}$ is a lattice, so let's see what we can do while we finish everything else.}
  \begin{rem}
...
This section proves Theorem~\ref{thm:TLbases}, which follows from the triangularity established in the next result.  Recall the $321$-avoiding permutations $T_{\lambda}$ defined in Section~\ref{sec:ex2} for each noncrossing partition $\lambda \in \NCP_{n}$.
