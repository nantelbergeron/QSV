...
$$H_n(q) = \sum\lambda q^{\ell(\lambda)}$$
corresponds to the (graded) quotient $R_n\big/\langle \QSym_{n}^{+} \rangle$.
\end{rem}

\begin{rem}
In light of Remark~\ref{rem:altECs}, one may observe that the image $\omega_0 \QSV_n\omega_0$ of the set $\QSV_n$ under the automorphism in Remark~\ref{rem:auto} has similar properties. In particular, the polynomials $P_\alpha$ can be modified to polynomials $P'_\alpha$ that vanish on every permutation of $\omega_0 \QSV_n\omega_0$. The top homogeneous component of $P'_\alpha$ would also be quasisymmetric, showing an analogous result for $\omega_0 \QSV_n\omega_0$. This is closely related to the automorphisms of the ring of quasisymmetric functions (see, for example,~\cite{JWY}).
\end{rem}
