...
\]
where $\mathbf{a}$, $\mathbf{b}$, $\mathbf{c}$, and $\mathbf{d}$ are (possibly empty) subwords of $w$ in one-line notation.  The quotient $\CC S_{n} / \ker(\phi)$ is then defined by the relations
\begin{multline}
...
\[
R_{n} / \mathsf{gr}(I_{n}) \cong R_{n}/I_{n},
\]
