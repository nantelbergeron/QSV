...
\]
so that $I_{n} = \mathbf{I}(S_{n})$, $\mathsf{gr}(I_{n}) = \langle \Sym_{n}^{+} \rangle$, and $R_{n}\big/\langle \Sym_{n}^{+} \rangle \cong R_{n}\big/\mathbf{I}(S_{n})$ as vector spaces.  
...
These results are reminiscent of Reading's Cambrian congruences~\cite{Reading1, Reading2} but there is no direct connection between the two; see Remark~\ref{rem:reading}.  However, we expect the combinatorics of excedance classes to be similarly rich, and there is much left to explore.   
...
We are also grateful to R.~Gonz\'alez~D'L\'eon for his inspiring discussions, and to N.~Reading for helping us to identify the disapointing result mentioned in Remark~\ref{rem:reading}.% related to Section~\ref{sec:weakorder}. 
...
\item Our excedance relation refines an equivalence relation defined in~\cite{ES} in which permutations are related if they share the same excedance position set.  
...
\begin{rem}
\label{rem:reading}
Our results are superficially similar to Reading's Cambrian congruences on the weak order~\cite{Reading1}.  
These congruences partition $S_{n}$ into intervals in the weak order with distinguished maximum and minimum elements, and two of the most natural examples have minimum elements that respectively avoid the patterns $231$ and $312$~\cite{Reading2}. 
Unfortunately there is no obvious one-to-one correspondence; by way of example the excedance class 
\[
\mathcal{C}_{\begin{tikzpicture}[scale = 0.35, baseline = 0.35*-0.2]
\foreach \x in {1, ..., 4}{\draw[fill] (\x - 1, 0) circle (2pt) node[inner sep = 2pt] (\x) {};}
\foreach \x in {1, ..., 4}{\node[below] at (\x) {$\scriptstyle \x$};}
\foreach \i\j in {1/4, 2/3}{\draw[thick] (\i) to[out = 35, in = 145] (\j);}
\end{tikzpicture}} = \{ \stackrel{1}{3}\stackrel{2}{4}\color{red}\stackrel{3}{1}\stackrel{4}{2}\color{black}, \stackrel{1}{4}\stackrel{2}{3}\color{red}\stackrel{3}{1}\stackrel{4}{2}\color{black}, \stackrel{1}{3}\stackrel{2}{4}\color{red}\stackrel{3}{2}\stackrel{4}{1}\color{black}, \stackrel{1}{4}\stackrel{2}{3}\color{red}\stackrel{3}{2}\stackrel{4}{1}\color{black} \}
\]
contains two $312$-avoiding permutations and two $231$-avoiding permutations.  Nonetheless, there may be a more subtle connection, and this prospect warrants further investigation.
\end{rem}
...
%, and 
%
% in~\cite{Reading1} has a dis
%
%
%is indexed by the 
%
%, Reading defines a family of quotients on the weak order of $S_{n}$ parametrized by Coxeter elements $c$.  
%The equivalence classes are intervals in the weak order with distinguished bottom and top elements which for the most natural choices of $c$ are respectively the $312$-avoiding and $231$-avoiding permutations; see~\cite{Reading2}.  
%Despite this remarkable similarity, there does not appear to be a direct connection between our results and Reading's: the excedance class 
%\[
%\mathcal{C}_{\begin{tikzpicture}[scale = 0.35, baseline = 0.35*-0.2]
%\foreach \x in {1, ..., 4}{\draw[fill] (\x - 1, 0) circle (2pt) node[inner sep = 2pt] (\x) {};}
%\foreach \x in {1, ..., 4}{\node[below] at (\x) {$\scriptstyle \x$};}
...
%\Lucas{What do you think of this remark?}

%\Nantel{Add remark about: Reading paper, bijection, c-sortable element is a section. our equivalence relation show these bijection.}
...
\bibitem{ABB} J.~C.~Aval, F.~Bergeron, and N.~Bergeron,
...
\bibitem{Baine}  P.~Biane, \textit{Some properties of crossings and partitions}. Discrete Math. {\bf 175} (1997) 41-53.

\bibitem{BjornerBrenti}  F.~Brenti and A.~Bj\"{o}rner, \textit{Combinatorics of Coxeter Groups}. Germany: Springer Berlin Heidelberg, 2010.
...
\bibitem{GobetWilliams}  T.~Gobet and N.~Williams,  \textit{Noncrossing partitions and Bruhat order}. Eur. J. Comb. {\bf 53} (2016) 8-34.

\bibitem{Gobet}  T.~Gobet,  \textit{Coxeter--Catalan Combinatorics and Temperley--Lieb Algebras}. Algebras Represent. Theory {\bf 24} (2021) 169–201.
...
\bibitem{Reading1}  N.~Reading, 
\textit{Cambrian Lattices},
Adv.~Math.~{\bf 205} (2006), 313–353.

\bibitem{Reading2}  N.~Reading, 
\textit{Clusters, Coxeter-Sortable elements and noncrossing partitions},
Trans.~Amer.~Math.~Soc.~{\bf 35} (2007), no.~12, 5931–5958.
...
\bibitem{Zinno}  M.~Zinno.  \textit{A Temperley--Lieb basis coming from the Braid group}. Journal of Knot Theory and Its Ramifications 11, {\bf 4} (2002) 575-599.
