...
\[
\{i \;|\; \text{$i \in [k-1]$ and $i \notin \EV(w)$} \} \supseteq \{w_{j} \;|\; \text{$j \in [k]$ and $j \notin \EP(w)$}\},
\]
...
Let $C$ denote the connected component of $\lambda$ containing $q_{i}$.  
From the definition of $Q_{\lambda}$, $q_{i}$ must be the maximal element of $C$,  and $Q_{\lambda}^{-1}(q_{i}) \le k$ the minimal element.  
...
\]
It is therefore sufficient to show that the above quantity $i-1$, so that $p_{i}$ is bounded above by each of $y_{i}, y_{i+1}, \ldots, y_{s}$, and in particular $p_{i} \le y_{i}$.
...
While most results in this section concern the Bruhat order defined in Section~\ref{sec:bruhat}, another widely used order on $S_{n}$ is the (left) weak order $\le_{L}$ (for example, see~\cite[Chapter 3]{BjornerBrenti}, ~\cite{A,B}\lucas{do you \\ think  \\ more \\ citations \\ are \\ needed?}).  This order is generated by the covering relations
\[
...
However, unlike the Bruhat order, the two posets are not isomorphic, and the order on the quotient is more amenable to our current and future work. 
