...
\]
where $w' < w$ is an element of $\CCC_{\lambda}$, $\sim$ denotes the excedance relation, and each coefficient $b_{w''}$ belongs to $\{1, 0, -1\}$.  
...
We are now ready to state and prove one of our main theorems. Let $R=\QQ[x_1,x_2,\ldots,x_n]$ denote the polynomial ring in $n$ variables.

\begin{thm}\label{thm:vanishingQSV} The Ideal $I=\langle P_\alpha : \alpha\models d>1\rangle\subseteq R$ is the vanishing ideal of $QSV_n$. \end{thm}

\begin{proof}
	Let $J$ denote the vanishing ideal of $QSV_n$. From Theorem~\ref{thm:vanishing}, we have that $I\subseteq J$. From this we deduce that
	 $$ \dim(R/I) \ge \dim(R/J)\,.$$
	 Since $J$ is the vanishing ideal of a finite set of points, we have 
	 $$ \dim(R/J)=|QSV_n|=C_n\,.$$
	 
	Let $I'=\langle M_\alpha : \alpha\models d>1\rangle$ and $gr(I)=\langle h(f) : f\in I \rangle$, where $h(f)$ is the top degree homogeneous component of $f$.
	For example, as noted in Definition~\ref{def:vanishP}, we have $h(P_\alpha)=M_\alpha$. Hence $I'\subseteq gr(I)$. 
	A standard use of Gr\"obner basis~\cite{CLO} shows that for graded-lex order, the initial ideal of $I$ is the same as the initial ideal of $gr(I)$ (they have the same leading term ideal).
	This gives us $\dim(R/gr(I)) =  \dim(R/I)$. Now, the main result of~\cite{ABB} shows that
	$ \dim(R/I')=C_n\,.$
	Thus, putting it all together, we have 
	$$C_n= \dim(R/I') \ge \dim(R/gr(I)) =  \dim(R/I) \ge \dim(R/J) =C_n\,,$$
	and equalities must hold everywhere. Therefor $I=J$.
\end{proof}
