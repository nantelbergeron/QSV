...
The importance of $\QSym$ has continuted to increase: in recent years quasisymmetric functions have been at the center of several research programs\lucas{citation \\ needed?}, and~\cite{ABS} establishes $\QSym$ as a universal setting for enumerative combinatorial invariants.
...
Similarly, the quasisymmetric polynomials $\QSym_{n}$ in $R_{n} = \QQ[x_{1}, \ldots, x_{n}]$ are the invariants of the Hivert action~\cite{Hivert} on $R_{n}$ 
\lucas{`Hivert action' \\ or `an action \\ defined by \\ Hivert?'} 
by the the Temperley--Lieb algebra $\TL_{n}(2)$.  
Writing $J_{n} = \langle \QSym_{n}^{+} \rangle$ 
\lucas{Do we \\ need to \\ name this \\ ideal? \\ What about \\ just $\langle \QSym_{n}^{+} \rangle$}
for ideal generated by the positive degree quasisymmetric polynomials,~\cite{AB,ABB} show that the dimension of the coinvariant space $R_{n}/J_{n}$ and $\TL_{n}(2)$ agree: both are the $n$th Catalan number $C_{n}$.  
Since $\TL_{n}(2)$ shares many nice properties with reflection groups, one might expect a Chevalley-type theorem from this coincidence, but there is no obvious $\TL_{n}(2)$-action on $R_{n}/J_{n}$:  Hivert's action is not multiplicative and $J_{n}$ is not a $\TL_{n}(2)$-submodule.  
...
On the other hand, module (2) comes from the left multiplicative action of $S_{n}$ on the permutahedron.\lucas{\raggedleft citation \\ needed?}  
However, careful inspection reveals that these modules determine one another!  
Consider the ideal
\[
I' = \langle f(x_{1}, \ldots, x_{n}) - f(1, \ldots, n) \;|\; f \in \Sym_{n}^{+} \rangle \subseteq \mathbf{I}(S_{n}).
\]
For each $f \in R_{n}$, let $\mathsf{h}(f)$ denote the top-degree homogeneous component of $f$, for any ideal $I$ in $R_{n}$ write $\mathsf{gr}(I) = \langle \mathsf{h}(f) \;|\; f \in I \rangle$.  Then $\mathsf{gr}(I') = \langle \Sym_{n}^{+} \rangle$, and from this equality Gr\"{o}bner basis theory (see e.g~\cite{CLO}) gives a linear isomorphism $R_{n}/\langle \Sym_{n}^{+} \rangle \cong R_{n}/I'$.  We therefore have
\[
|S_{n}| = \dim(R_{n}/\langle \Sym_{n}^{+} \rangle) = \dim(R_{n}/I') \ge \dim(R_{n}/\mathbf{I}(S_{n})) = |S_{n}|,
\]
so that $I' = \mathbf{I}(S_{n})$ and the above linear isomorphism becomes $R_{n}/\langle \Sym_{n}^{+} \rangle \cong R_{n}/\mathbf{I}(S_{n})$.  
...
Thus, we have an $S_{n}$-module isomorphism $R_{n}/\langle \Sym_{n}^{+} \rangle \cong R_{n}/\mathbf{I}(S_{n})$.  
...
It is known  that $\langle \Sym_{n}^{+} \rangle \subseteq J_{n}$, and that there is a surjective algebra homomorphism $\phi: \CC S_{n} \to \TL_{n}(2)$. 
...
\item considering the vanishing ideal $\mathbf{I}(\QSV_{n})$, we have $\mathsf{gr}(\mathbf{I}(\QSV_{n})) = J_{n}$.

\end{enumerate}
Assuming such a set exists, one can define an action of $\TL_{n}(2)$ on the space $R_{n}/J_{n}$ using Gr\"{o}bner basis theory and the multiplication constants for the basis obtained from $\QSV_{n}$.  
...
\[
J_n = \langle \QSym_{n}^{+} \rangle = \mathsf{gr}(\mathbf{I}(\QSV_{n})).
\]
...
\[
R_{n}/\mathbf{I}(\QSV_{n}) \cong R_{n}/J_{n}.
\]
Future work will explore the module structure on $R_{n}/J_{n}$ implicit in this result.
