...
\begin{proof}
 ...
 is $C(S)=\{j : S\cap C_j \not= \emptyset \}$. We have
 	$P_\alpha=\sum_{T\subseteq [r]} P_{\alpha,T},$
where 
...
\medskip 
\noindent{\bf Case $\bf |T|>1$}: \nantel{I am not done this part. need to add much more details}
When we have more than one cycle involved, let $T=\{t_1,t_2,\ldots,t_r\}$ and we assume (without lost of generality) that $C=C_{t_1}$
is a cycle that do not contain (in the non-crossing sense) any nested other cycles of the $C_{t_i}$ for $i>1$. The fact that the cycles of $\sigma$ are non-crossing guaranties the existence of such $C$
for any given $T$.  We now partition the terms of Equation~\eqref{eq:PT} according to the intersection of $ i_1<i_2<\cdots < i_\ell$ with the $C_{t_i}$ for $i>1$.
We will see that the portion of the terms intersecting $C$ will be a vanishing polynomials as in the case $|T|=1$.
